...
| **HD**   | Hit Dice. Number of d8 hit dice, average HP in parentheses. `X+Y` / `X−Y` adds or subtracts from the rolled total (min 1 HP). `HD ½` counts as 1 HD for any rule referencing HD. Each `*` after HD marks one notable special ability. |
